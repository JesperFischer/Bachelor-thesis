...
function [logp, yhat, res] = tgi_joint_main_effects_sa3hat_all(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*rts+be2.*stim(:,2)+be3.*stim(:,4)+be4.*stim(:,3)+be5.*sa3hat+be6.*stim(:,4).*sa3hat+be7.*stim(:,2).*sa3hat+be8.*sa3hat.*stim(:,3);
