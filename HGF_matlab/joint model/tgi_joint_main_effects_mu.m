...
function [logp, yhat, res] = tgi_joint_main_effects_mu(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*mu3+be6.*stim(:,2).*mu3+be7.*stim(:,3).*mu3+be8.*stim(:,4).*mu3;
