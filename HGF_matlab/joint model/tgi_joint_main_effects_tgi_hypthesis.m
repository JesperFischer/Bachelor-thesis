...
function [logp, yhat, res] = tgi_joint_main_effects_tgi_hypthesis(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*rts+be2.*stim(:,2)+be3.*stim(:,3)+be4.*stim(:,4)+be5.*envi+be6.*sa3hat+be7.*stim(:,2).*envi+be8.*stim(:,3).*envi+be9.*stim(:,4).*envi+be10.*stim(:,2).*sa3hat+be11.*stim(:,3).*sa3hat+be12.*stim(:,4).*sa3hat;
