...
function [logp, yhat, res] = tgi_joint_main_effects_tgi_sa2hat_esti(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*rts+be2.*stim(:,2)+be3.*stim(:,3)+be4.*stim(:,4)+be5.*esti+be6.*stim(:,2).*esti+be7.*stim(:,3).*esti+be8.*stim(:,4).*esti;
