...
function [logp, yhat, res] = tgi_joint_main_effects_tgi_surprise2(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*rts+be2.*stim(:,2)+be3.*stim(:,3)+be4.*stim(:,4)+be5.*pe1+be6.*stim(:,2).*pe1+be7.*stim(:,3).*pe1+be8.*stim(:,4).*pe1;
