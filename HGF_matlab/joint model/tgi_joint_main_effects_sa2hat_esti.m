...
function [logp, yhat, res] = tgi_joint_main_effects_sa2hat_esti(r, infStates, ptrans)
...
esti = circshift(sa2,1);
esti(1) = 0.1; %prior used
esti(r.irr2) = [];
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*esti+be6.*stim(:,2).*esti+be7.*stim(:,3).*esti+be8.*stim(:,4).*esti;
