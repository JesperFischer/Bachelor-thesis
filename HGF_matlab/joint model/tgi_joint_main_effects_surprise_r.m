...
function [logp, yhat, res] = tgi_joint_main_effects_surprise(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*surp+be6.*stim(:,2).*surp+be7.*stim(:,3).*surp+be8.*stim(:,4).*surp;
