...
function [logp, yhat, res] = tgi_joint_main_effects_pred_sa3hat(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*predacc(:,3)+be4.*predacc(:,4)+be5.*sa3hat+be6.*predacc(:,2).*sa3hat+be7.*predacc(:,3).*sa3hat+be8.*predacc(:,4).*sa3hat+be9.*stim(:,3)+be10.*stim(:,4);
