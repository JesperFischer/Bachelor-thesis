...
function [logp, yhat, res] = tgi_joint_main_effects_alles_mu2(r, infStates, ptrans)
...
for i = 1:length(predacc);
if predacc(i,1) == 0;
    predacc(i,2) = 1;
    predacc(i,3) = 0;
    predacc(i,4) = 0;
elseif predacc(i,1) == 1;
    predacc(i,2) = 0;
    predacc(i,3) = 1;
    predacc(i,4) = 0;
else
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*inferv+be6.*mu2hat+be7.*mu2hat.*inferv+be8.*stim(:,2).*mu2hat.*inferv+be9.*stim(:,3).*mu2hat.*inferv+be10.*stim(:,4).*mu2hat.*inferv;
