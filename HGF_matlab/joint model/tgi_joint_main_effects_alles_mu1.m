...
function [logp, yhat, res] = tgi_joint_main_effects_alles_mu1(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*sa1hat+be6.*mu1hat+be7.*mu1hat.*sa1hat+be8.*stim(:,2).*mu1hat.*sa1hat+be9.*stim(:,3).*mu1hat.*sa1hat+be10.*stim(:,4).*mu1hat.*sa1hat;
