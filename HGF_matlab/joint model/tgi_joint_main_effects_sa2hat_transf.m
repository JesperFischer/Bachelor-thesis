...
function [logp, yhat, res] = tgi_joint_main_effects_sa2hat_transf(r, infStates, ptrans)
...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
pv = tapas_sgm(mu2, 1).*(1-tapas_sgm(mu2, 1)).*exp(mu3hat); % transform down to 1st level
pv(r.irr2) = [];
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*pv+be6.*stim(:,2).*pv+be7.*stim(:,3).*pv+be8.*stim(:,4).*pv;
