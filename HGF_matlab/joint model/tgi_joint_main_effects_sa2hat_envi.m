...
function [logp, yhat, res] = tgi_joint_main_effects_sa2hat_envi(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*trial+be2.*rts+be3.*stim(:,3)+be4.*stim(:,4)+be5.*envi+be6.*stim(:,2).*envi+be7.*stim(:,3).*envi+be8.*stim(:,4).*envi;
