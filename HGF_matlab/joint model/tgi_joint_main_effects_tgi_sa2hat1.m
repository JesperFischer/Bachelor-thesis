...
function [logp, yhat, res] = tgi_joint_main_effects_tgi_sa2hat1(r, infStates, ptrans)
...
trial = trial';
trial(r.irr2) = [];
logrt = be0+be1.*rts+be2.*stim(:,2)+be3.*stim(:,3)+be4.*stim(:,4)+be5.*sa2hat+be6.*stim(:,2).*sa2hat+be7.*stim(:,3).*sa2hat+be8.*stim(:,4).*sa2hat;
